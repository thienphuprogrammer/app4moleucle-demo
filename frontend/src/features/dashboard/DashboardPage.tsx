...
import {
  Loader2, Sparkles, CheckCircle2, LayoutList, Box, Copy, Wand2,
  FileText, Beaker, Atom, Cpu, Info,
} from 'lucide-react';
...
import { AVAILABLE_MODELS, type GenerationRecord, type SingleModelResult } from '@/lib/types';
...
export function DashboardPage() {
  ...
  const [results, setResults] = useState<SingleModelResult[]>([]);
  ...
  const [inputMode, setInputMode] = useState<'text' | 'smiles'>('text');
  ...
  const fetchHistory = useCallback(async () => {
    try {
      const data = await moleculesApi.getHistory();
      setHistory(data);
    } catch (e) {
  ...
  const handleNewResult = (data: GenerationRecord, usedPrompt: string) => {
    setResults(data.results);
    if (data.results.length > 0) {
      const first = data.results[0];
      setActiveSmiles(first.smiles);
      setActiveModel(first.model_name);
      setActiveRecord({ ...data, prompt: usedPrompt });
  ...
  const handleGenerate = async () => {
    ...
    try {
      const data = await moleculesApi.generate({ prompt, models: selectedModels });
      handleNewResult(data, prompt);
    } catch (e) {
  ...
  const handleSmilesVisualize = () => {
    ...
    setResults([{ model_name: 'user_input', smiles: smilesInput.trim(), confidence: 1.0, execution_time: 0 }]);
    setActiveRecord({
      id: `smiles-${Date.now()}`,
      prompt: `SMILES: ${smilesInput.trim()}`,
      created_at: new Date().toISOString(),
      results: [{ model_name: 'user_input', smiles: smilesInput.trim(), confidence: 1.0, execution_time: 0 }],
    });
    setIsResultListOpen(true);
  ...
  const handleMol2Text = async () => {
    ...
    try {
      const res = await knowledgeApi.mol2text({ smiles: activeSmiles });
      if (res.success) {
        setMol2textResult(res.description);
  ...
  const handleHistoryClick = (record: GenerationRecord) => {
            ...
            {mode === 'generate' ? (
              ...
              >
                ...
                <div className="p-4 md:p-6 space-y-5 flex-1 overflow-y-auto">
                  <Tabs value={inputMode} onValueChange={(v) => setInputMode(v as 'text' | 'smiles')} className="w-full">
                    ...
                    <TabsContent value="smiles" className="mt-4 space-y-4">
                      ...
                      <div className="space-y-2">
                        <label className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Common Examples</label>
                        <div className="grid grid-cols-2 gap-2">
                          {[
                            ...
                            <Button key={ex.name} variant="outline" size="sm" className="text-xs justify-start" onClick={() => setSmilesInput(ex.smiles)}>
                              {ex.name}
                            </Button>
                          ))}
                        </div>
        ...
        >
          ...
          <div className="flex items-center justify-between mb-4 relative z-10">
            ...
            <div className="flex items-center gap-2">
              {activeSmiles && (
                <>
                  <Button variant="outline" size="sm" onClick={handleCopySmiles} className="h-8 text-xs font-semibold gap-2">
                    <Copy className="w-3.5 h-3.5" />
                    <span className="hidden sm:inline">Copy SMILES</span>
                  </Button>
                  <Button variant="outline" size="sm" onClick={handleMol2Text} disabled={mol2textLoading} className="h-8 text-xs font-semibold gap-2">
              ...
              {mode === 'generate' && results.length > 1 && (
                <Button variant="outline" size="sm" onClick={() => setIsResultListOpen(!isResultListOpen)} className="h-8 text-xs font-semibold gap-2">
          ...
          {activeSmiles ? (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full h-full flex flex-col md:flex-row gap-4 md:gap-6 relative">
              <div className="flex-1 flex flex-col gap-4 min-w-0 transition-all duration-300 h-full">
                ...
                {mol2textResult && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-card border border-border rounded-xl p-4 shadow-sm">
                    ...
                    <p className="text-sm text-foreground leading-relaxed whitespace-pre-wrap">{mol2textResult}</p>
                  </motion.div>
                )}
              </div>
              ...
              <AnimatePresence>
                {mode === 'generate' && results.length > 1 && isResultListOpen && (
                  ...
                  >
                    <div className="text-xs font-bold text-muted-foreground uppercase tracking-wider px-1">Model Comparison ({results.length} results)</div>
                    <div className="flex-1 overflow-y-auto pr-1 space-y-3 pb-2 scrollbar-hide">
                      {results.map((res, idx) => {
                        ...
                        const Icon = MODEL_ICONS[res.model_name] || Cpu;
                        ...
                        return (
                          <motion.div
                            ...
                            className={cn(
                              ...
                              isActive ? 'border-primary bg-primary/5 shadow-md scale-[1.02]' : 'border-border/50 bg-card hover:border-primary/50 hover:bg-muted/30'
                            )}
                            onClick={() => {
                            ...
                            <div className="p-3">
                              <div className="flex justify-between items-center mb-1">
                                ...
                                <div className="flex gap-1 items-center">
                                  <span className="text-[10px] text-muted-foreground">{(res.confidence * 100).toFixed(0)}%</span>
                                  {isActive && <CheckCircle2 className="w-3.5 h-3.5 text-primary" />}
                                </div>
                              </div>
                              <code className="text-[10px] font-mono text-muted-foreground block truncate">{res.smiles}</code>
                            </div>
                          </motion.div>
                        );
                      })}
